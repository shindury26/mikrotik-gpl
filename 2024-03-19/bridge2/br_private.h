...
enum {
	...
	BR_GROUPFWD_LLDP	= BIT(14),
};

#define BR_GROUPFWD_RESTRICTED (BR_GROUPFWD_STP | BR_GROUPFWD_MACPAUSE | \
				BR_GROUPFWD_LACP | BR_GROUPFWD_LLDP)
...
// XXX: Beware this structure is used by the kernel
struct net_bridge_port {
	...
	unsigned			horizon;
	unsigned			switch_group;
	unsigned			vlan_ingress;
	struct net_dhcp_info_opt __rcu	*info_option;
};
...
// XXX: Keep in mind that kernel netfilter code uses this structure
// TRY to keep the start of the structure as close as possible to the
// kernel
// DON'T mess up fake_rtable offset
struct net_bridge {
	spinlock_t			lock;
	...
	u8				group_addr[ETH_ALEN] __aligned(2);
	...
	unsigned			port_count;
	u8				allow_fast_forward:1;
	u8				fast_forward:1;
	u8				dhcp_snooping:1;
	u8				add_info_option:1;
...
//	u8 proxyarp_replied:1;
	...
	u8 src_port_peer_link:1;
	u8 src_port_hw_switched:1;
#ifdef CONFIG_BRIDGE_VLAN_FILTERING
	u8 vlan_filtered:1;
#endif
...
void br_fdb_flush_bridge_local(struct net_bridge *br);
...
void br_multicast_flood(struct net_bridge_mdb_entry *mdst,
			struct sk_buff *skb, bool local_rcv, bool local_orig, unsigned horizon, unsigned switch_group);
...
/* br_vlan.c */
struct sk_buff *br_handle_vlan_switch(struct net_bridge *br, struct sk_buff *skb);
bool br_allowed_ingress(const struct net_bridge *br,
			struct net_bridge_vlan_group *vg, unsigned vlan_ingress,
...
static inline unsigned long br_timer_value(const struct timer_list *timer)
{
...
static inline bool br_ipv6_enabled(void) {
	return true;
}
...
extern struct ratelimit_state br_warn_ratelimit_state;
extern unsigned iface_warning_topics[];
extern unsigned bridge_warning_topics[];
