...
static inline size_t br_port_info_size(void)
{
	return nla_total_size(1)	/* IFLA_BRPORT_STATE  */
		...
		+ nla_total_size(sizeof(u16))	/* IFLA_BRPORT_GROUP_FWD_MASK */
	        + nla_total_size(1)	/* IFLA_BRPORT_VLAN_DEF_STATE */
		+ 0;
}
...
static const struct nla_policy br_port_policy[IFLA_BRPORT_MAX + 1] = {
	...
	[IFLA_BRPORT_VLAN_DEF_STATE]	= { .type = NLA_U8 },
};
...
//	}
	...
	if (tb[IFLA_BRPORT_VLAN_DEF_STATE]) {
		unsigned state = nla_get_u8(tb[IFLA_BRPORT_VLAN_DEF_STATE]);

		if (state > BR_STATE_BLOCKING)
			return -EINVAL;
		p->vlan_def_state = state;
	}
...
//	    nla_put_u16(skb, IFLA_BR_PRIORITY, priority) ||
	if (nla_put_u8(skb, IFLA_BR_VLAN_FILTERING, vlan_enabled) ||
