...
static struct fp_buf *cdc_mbim_process_dgram(struct usbnet *dev, u8 *buf, size_t len, u16 tci)
{
	__be16 proto = __constant_htons(ETH_P_802_3);
	struct fp_buf *fpb = NULL;
        struct ethhdr *ehdr;
        unsigned char *data;
	...
	if (tci < 256 || tci == MBIM_IPS0_VID) { /* IPS session? */
        ...
        fpb = __fpb_alloc(len + ETH_HLEN + sizeof(struct vlan_hdr));
        if (!fpb)
            goto err;

	ehdr = (struct ethhdr*) fpb_data(fpb);
        ehdr->h_proto = __constant_htons(ETH_P_8021Q);
	memcpy(ehdr->h_dest, dev->net->dev_addr, ETH_ALEN);
	memcpy(ehdr->h_source, dev->net->dev_addr, ETH_ALEN);

        data = (unsigned char*) (ehdr + 1);

        if (likely(tci)) {
            struct vlan_hdr *vhdr = (struct vlan_hdr *) data;
            vhdr->h_vlan_TCI = htons(tci);
            vhdr->h_vlan_encapsulated_proto = proto;
