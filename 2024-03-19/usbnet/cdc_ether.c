...
extern const struct driver_info	simcom_info;
...
skip:
	...
	if (info->control->cur_altsetting->desc.bNumEndpoints == 1) {
		...
		if (!usb_endpoint_is_int_in(desc) ||
		    ...
		    !desc->bInterval) {
			dev_err(&intf->dev, "bad notification endpoint\n");
			dev->status = NULL;
		}
	}
	if (rndis && !dev->status) {
...
void usbnet_cdc_status(struct usbnet *dev, struct urb *urb)
{
	...
	case USB_CDC_NOTIFY_NETWORK_CONNECTION:
		netif_info(dev, link, dev->net, "CDC: carrier %s\n",
			  event->wValue ? "on" : "off");
 ...
 */
        ...
        {USB_DEVICE(0x1e0e, 0x9025), .driver_info = (unsigned long)&simcom_info },
        {USB_DEVICE(0x1e0e, 0x9000), .driver_info = (unsigned long)&simcom_info },
        {USB_DEVICE(0x1e0e, 0x9001), .driver_info = (unsigned long)&simcom_info },
        {USB_DEVICE(0x1e0e, 0x9002), .driver_info = (unsigned long)&simcom_info },
        {USB_DEVICE(0x1e0e, 0x901a), .driver_info = (unsigned long)&simcom_info },
        {USB_DEVICE(0x1e0e, 0x901b), .driver_info = (unsigned long)&simcom_info },
{
