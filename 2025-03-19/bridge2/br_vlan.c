...
struct sk_buff *br_handle_vlan_switch(struct net_bridge *br, struct sk_buff *skb)
{
	...
	/* tag frames that are headed to the switch chip so that the vlan
	 * information ins't lost in case case when the switch_master is in the
	 * untagged vlan bitmap */
...
static bool br_allowed_frame_type(unsigned vlan_ingress, bool tagged) {
	unsigned frame_types = vlan_ingress & BR_IN_FRAME_TYPES;
	if (!tagged && frame_types == BR_PORT_FRAME_ADMIT_TAGGED) {
		return false;
	} else if (tagged && frame_types == BR_PORT_FRAME_ADMIT_UNTAGGED) {
		return false;
	}
	return true;
}
			      ...
			      u8 *state)
{
	...
	if (!br_vlan_get_tag(skb, vid)) {
		/* Tagged frame */
		if (skb->vlan_proto != br->vlan_proto || vlan_ingress & BR_IN_TAG_STACKING) {
	...
	if (!BR_INPUT_SKB_CB(skb)->src_port_hw_switched
	        && !br_allowed_frame_type(vlan_ingress, tagged))
		goto drop;
	...
	if (vlan_ingress & BR_IN_FILTERING && !vlan_ok)
		goto drop;
			...
			struct sk_buff *skb, u16 *vid, u8 *state)
{
	...
	return __allowed_ingress(br, vg, vlan_ingress, skb, vid, state);
}
