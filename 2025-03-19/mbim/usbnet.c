...
#include "../../../kernel/linux6/net/8021q/vlan.h"
#include "usbnet.h"
#include "../usbnet/io.h"
 ...
 * the equation.
 */
#define	BIG_PACKET	        (1518)
#define	MAX_QUEUE_MEMORY	(60 * BIG_PACKET)
...
void usbnet_skb_return (struct usbnet *dev, struct fp_buf *fpb)
{
	...
	netif_dbg(dev, rx_status, dev->net, "< rx, len %zu\n",
		  fpb_len(fpb) + sizeof (struct ethhdr));

        if (list_empty(&br_list)) {
            fast_path_rx_noinvalidate(dev->net, fpb);
            return;
        }
        ...
        list_for_each_entry_rcu(br, &br_list, list) {
            if (br->modem != dev->net) continue;
            if (br->vlan_id != vlan_id) continue;
            ...
            if (!br->slave) { // local
                ...
                struct net_device *dst_dev = br->slave;
                memcpy(eth->h_dest, br->dst_mac, ETH_ALEN);

                fast_path_fp_rx_inc(br->vlan, fpb);
                rcu_read_unlock();
		...
		struct sk_buff_head *list, enum skb_state state)
{
	...
	struct skb_data *entry = (struct skb_data *) skb->cb;
        struct sk_buff_head *queue = (state == tx_done) ? &dev->tx_done : &dev->done;
	 ...
	 */
	spin_lock_nested(&queue->lock, SINGLE_DEPTH_NESTING);
...
{
	if (likely(dev->driver_info->rx_fixup)) {
            int frames = dev->driver_info->rx_fixup(dev, skb);
            if (!frames) {
...
static void usbnet_terminate_urbs(struct usbnet *dev)
{
	...
	wait_skb_queue_empty(&dev->tx_done);
	netif_dbg(dev, ifdown, dev->net,
...
int usbnet_open (struct net_device *net)
{
	...
	struct driver_info	*info = dev->driver_info;
        ...
        skb_bin_request(net, USB_CDC_NCM_NTB_MIN_IN_SIZE - SKB_SHARED_INFO_SIZE - NET_SKB_PAD);
	...
	netif_info(dev, ifup, dev->net,
		   "open: enable queueing (rx %d, tx %d, budget %u) mtu %d %s framing\n",
		   (int)RX_QLEN(dev), (int)TX_QLEN(dev),
                   RX_QLEN(dev) * dev->rx_urb_size / BIG_PACKET,
		   dev->net->mtu,
...
static struct net_device *passthrough_fwd_handler(struct net_device *in_dev, struct fp_buf *fpb) {
    ...
    struct br_entry *br;
    struct net_device *vlan, *modem = NULL;
    int vlan_id = 0;
    ...
    if (proto == __constant_htons(ETH_P_IP)) {
        ...
        if (ip->protocol == IPPROTO_UDP) {
            ...
            // rx DHCP locally if not relayed
    ...
    // rx VLAN locally
    } else if (ANY_VLAN_PROTO_N(proto) && !(in_dev->priv_flags & IFF_802_1Q_VLAN)) {
        return NULL;
    ...
    struct sk_buff *skb = fpb_to_skb(fpb);
    if (!skb) {
        modem->stats.tx_dropped++;
	__fpb_free(fpb);
        return FAST_PATH_CONSUMED;
    }

    fast_path_fp_tx_inc(vlan, fpb);

    __vlan_hwaccel_put_tag(skb, htons(ETH_P_8021Q), vlan_id);

    skb->dev = modem;
    usbnet_start_xmit(skb, modem);
    return FAST_PATH_CONSUMED;
}
				     ...
				     struct net_device *net)
{
	...
	case 0:
		...
		if (dev->txq.qlen >= TX_QLEN (dev) || dev->tx_done.qlen >= TX_QLEN(dev))
			netif_stop_queue (net);
	}
	spin_unlock_irqrestore (&dev->txq.lock, flags);
...
static int rx_alloc_submit(struct usbnet *dev, gfp_t flags)
{
	...
	int		ret = 0;
        ...
        if (dev->done.qlen > RX_QLEN(dev)) goto err;
...
static void usbnet_bh (unsigned long param)
{
	...
	while ((skb = skb_dequeue (&dev->done))) {
		...
		case rx_done:
			...
			frames += rx_process (dev, skb);
                        // cleanup
			usb_free_urb (entry->urb);
                        consume_skb (skb);
                        if (frames > BH_BUDGET) goto defer;
			continue;

		case tx_done: // falls through
			kfree(entry->urb->sg);
		case rx_cleanup:
			usb_free_urb (entry->urb);
			consume_skb (skb);
...
defer:
	...
	while ((skb = skb_dequeue(&dev->tx_done))) {
                entry = (struct skb_data *) skb->cb;
                kfree(entry->urb->sg);
                usb_free_urb (entry->urb);
                consume_skb (skb);
        ...
        // don't hog, allow NAPI too
        if (likely(frames > BH_BUDGET)) {
            if (dev->done.qlen > RX_QLEN(dev)) {
                tasklet_schedule (&dev->bh);
                return;
            }
        }
	...
	// or are we maybe short a few urbs?
        ...
        if (dev->done.qlen) tasklet_schedule (&dev->bh);
}
...
static int unregister_slave(struct net_device *slave, struct net_device *modem) {
    ...
    list_for_each_entry_safe(br, tmp, &br_list, list) {
        if (slave) {
            if (br->slave != slave && br->vlan != slave) continue;
        } else {
            if (br->modem != modem) continue;
        }

        if (slave) printk("usbnet: slave %s unreg\n", slave->name);
        else printk("usbnet: master %s unreg\n", modem->name);

        if (br->slave) unregister_fast_path(&fp_rx_handler, br->slave);

        list_del_rcu(&br->list);
        synchronize_rcu();

        if (br->slave) dev_put(br->slave);
        if (br->vlan) dev_put(br->vlan);
        kfree(br);
...
int usbnet_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd) {
    ...
    if (!p.add) {
        ...
        struct net_device *slave = NULL, *vlan = NULL;
        if (p.slave_id) {
            slave = dev_get_by_index(&init_net, p.slave_id);
            if (!slave) return -ENODEV;

            vlan = vlan_find_dev(dev, htons(ETH_P_8021Q), p.vlan_id);
            if (!vlan) {
                dev_put(slave);
                return -ENODEV;
            }
        ...
        list_for_each_entry(br, &br_list, list) {
            if (br->modem != dev) continue;
            if (slave && br->slave == slave) {
                if (slave) dev_put(slave);
                return -EEXIST;
            }
        }
        ...
        else printk("usbnet: passthrough %s -> local\n", dev->name);
...
usbnet_probe (struct usb_interface *udev, const struct usb_device_id *prod)
{
	...
	dev->msg_enable = netif_msg_init (msg_level, NETIF_MSG_DRV | NETIF_MSG_IFUP
...
#define netif_dbg(priv, type, dev, format, args...)             \
        printk(format, ##args)
#undef dev_warn
#define dev_warn(priv, format, args...) \
        printk(format, ##args)
#undef dev_dbg
#define dev_dbg(priv, format, args...) \
        printk(format, ##args)
#endif
...
static int __init usbnet_init(void)
{
	...
	eth_random_addr(node_id);
        ...
        usb_register(&wdm_driver);
        usb_register(&cdc_ncm_driver);
        usb_register(&cdc_mbim_driver);
