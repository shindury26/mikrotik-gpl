...
static int msm_gpio_get_cd(struct mmc_host *host)
{
	return !gpio_get_value_cansleep(cd_gpio_pin);
}
...
static void sdhci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	present = msm_gpio_get_cd(host->mmc);
...
{
	int gpio_cd = msm_gpio_get_cd(host->mmc);
...
void sdhci_card_event(struct mmc_host *mmc)
{
	struct sdhci_host *host = mmc_priv(mmc);
	unsigned long flags;
	int present;
