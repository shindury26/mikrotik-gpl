...
static void br_update_switch_flooding(struct net_bridge *br, bool ipv4, unsigned delay) {
	unsigned switches = 0;
	struct net_bridge_port *p;

	// We haven't reveiced query, but we can be a multicast_querier, so
	// update the info, because it might not have been set yet
	rcu_read_lock();
	list_for_each_entry_rcu(p, &br->port_list, list) {
		if (p->switch_group == BR_NOSWITCH || (switches & BIT(p->switch_group))) continue;
		switches |= BIT(p->switch_group);
		switch_sched_mc_flood_update(p->dev, ipv4,
				br_opt_get(br, BROPT_MULTICAST_QUERIER), delay);
	}
	rcu_read_unlock();
					...
					struct sk_buff *skb, u16 vid)
{
	...
	} else if (br_ipv6_enabled() && skb->protocol == htons(ETH_P_IPV6)) {
...
static void br_ip4_multicast_querier_expired(struct timer_list *t)
{
	...
	br_update_switch_flooding(br, true, 0);
}
				      ...
				      struct br_ip *ip)
{
	...
	BR_INPUT_SKB_CB(skb)->igmp = igmp_type;
...
void br_multicast_del_port(struct net_bridge_port *port)
{
	...
	hlist_for_each_entry_safe(pg, n, &port->mglist, mglist)
		br_multicast_del_pg(br, pg);
	if (rcu_dereference(br->ip4_querier.port) == port) {
	    rcu_assign_pointer(br->ip4_querier.port, NULL);
	}
	if (rcu_dereference(br->ip6_querier.port) == port) {
	    rcu_assign_pointer(br->ip6_querier.port, NULL);
	}
...
static bool br_multicast_select_querier(struct net_bridge *br,
					...
					struct br_ip *saddr)
{
	if (saddr->proto == htons(ETH_P_IP))
		return br_ip4_multicast_select_querier(br, port, saddr->u.ip4);
	else if (br_ipv6_enabled() && saddr->proto == htons(ETH_P_IPV6))
				    ...
				    struct net_bridge_port *port)
{
	...
	if (port && port->switch_group != BR_NOSWITCH)
					...
					unsigned long max_delay)
{
	...
	if (port && port->switch_group != BR_NOSWITCH && !timer_pending(&query->timer)) {
		switch_sched_mc_flood_update(port->dev,
				saddr->proto == htons(ETH_P_IP), true,
				max_delay);
	}
				 ...
				 u16 vid, bool inject)
{
	...
	if (err == -ENOMSG) {
		if (!ipv4_is_local_multicast(ip_hdr(skb)->daddr)) {
			...
			BR_INPUT_SKB_CB(skb)->ll_mc
				= __br_multicast_querier_exists(br, &br->ip4_other_query, false);
		}
				 ...
				 u16 vid, bool inject)
{
	...
	if (err == -ENOMSG) {
		if (!ipv6_addr_is_ll_all_nodes(&ipv6_hdr(skb)->daddr))
			...
			BR_INPUT_SKB_CB(skb)->ll_mc
				= __br_multicast_querier_exists(br, &br->ip6_other_query, true);
...
int br_multicast_rcv(struct net_bridge *br, struct net_bridge_port *port,
		     struct sk_buff *skb, u16 vid, bool inject)
{
	int ret = 0;
	...
	if (skb->protocol == htons(ETH_P_IP)) {
		ret = br_multicast_ipv4_rcv(br, port, skb, vid, inject);
	} else if (br_ipv6_enabled() && skb->protocol == htons(ETH_P_IPV6)) {
		ret = br_multicast_ipv6_rcv(br, port, skb, vid, inject);
	}
...
static void __del_port_router(struct net_bridge_port *p)
{
	...
	if (p && p->switch_group != BR_NOSWITCH)
...
unlock:
	br_update_switch_flooding(br, true, max_delay);
	if (br_ipv6_enabled())
		br_update_switch_flooding(br, false, max_delay);
	spin_unlock_bh(&br->multicast_lock);
...
static void br_multicast_offload_pg(struct net_bridge *br,
				    struct net_bridge_port_group *pg,
				    bool offload)
{
       struct net_bridge_mdb_entry *mp;
       ...
       for (pp = &mp->ports;
            ...
            pp = &p->next) {
               if (p != pg)
                       continue;
               br_mdb_switchdev_notify(p->port, &p->addr, offload ? RTM_NEWMDB : RTM_DELMDB);
               return;
       }
}

void br_multicast_offload(struct net_bridge_port *port, bool offload)
{
       struct net_bridge *br = port->br;
       struct net_bridge_port_group *pg;
       ...
       spin_lock_bh(&br->multicast_lock);
       rcu_read_lock();
       hlist_for_each_entry_rcu(pg, &port->mglist, mglist)
               br_multicast_offload_pg(br, pg, offload);
       rcu_read_unlock();
       if (!hlist_unhashed(&port->rlist))
               br_port_mc_router_state_change(port, offload);
       spin_unlock_bh(&br->multicast_lock);

       bool own_querier = br_opt_get(br, BROPT_MULTICAST_QUERIER);
       bool ipv4 = timer_pending(&br->ip4_other_query.timer);
       bool ipv6 = timer_pending(&br->ip6_other_query.timer);
       unsigned long ipv4_delay = 0;
       unsigned long ipv6_delay = 0;
       if (time_is_after_jiffies(br->ip4_other_query.delay_time)) {
	       ipv4_delay = br->ip4_other_query.delay_time - jiffies;
       }
       if (time_is_after_jiffies(br->ip6_other_query.delay_time)) {
	       ipv6_delay = br->ip6_other_query.delay_time - jiffies;
       }
       unsigned long delay = max(ipv4_delay, ipv6_delay);
       if (offload) {
	       switch_sched_mc_flood_update(port->dev, true, own_querier || ipv4, delay);
	       switch_sched_mc_flood_update(port->dev, false, own_querier || ipv6, delay);
       }
       else {
           switch_sched_mc_flood_update(port->dev, true, false, 0);
           switch_sched_mc_flood_update(port->dev, false, false, 0);
       }
}
