...
#define VMXNET3_FPB_OFFSET NET_SKB_PAD
...
#define VMXNET3_FPB_FLAG       BIT(0)

static bool is_fpb_ptr(void *ptr)
{
	return (unsigned long)ptr & VMXNET3_FPB_FLAG;
}

static void *fpb_to_ptr(struct fp_buf *fpb)
{
	return (void *)((unsigned long)fpb_buf(fpb) | VMXNET3_FPB_FLAG);
}

static void *ptr_to_fpb_buf(void *ptr)
{
	return (void*)((unsigned long)ptr & ~VMXNET3_FPB_FLAG);
}

static void *skb_to_ptr(struct sk_buff *skb)
{
	return (void *)(skb);
}
...
{
	return (struct sk_buff *)(ptr);
}

static void vmxnet3_free_buff_ptr(void *ptr)
{
	if (is_fpb_ptr(ptr)) {
		skb_bin_put_buffer(ptr_to_fpb_buf(ptr));
	} else {
		dev_kfree_skb_any(ptr_to_skb(ptr));
	}
}
		  ...
		  struct pci_dev *pdev,	struct vmxnet3_adapter *adapter)
{
	...
	ptr = tq->buf_info[eop_idx].ptr;
	BUG_ON(!ptr);
	tq->buf_info[eop_idx].ptr = NULL;
			...
			int num_to_alloc, struct vmxnet3_adapter *adapter)
{
	...
	while (num_allocated <= num_to_alloc) {
		...
		if (rbi->buf_type == VMXNET3_RX_BUF_SKB) {
			if (rbi->buffer == NULL) {
				rbi->buffer = skb_bin_alloc_buffer(GFP_KERNEL);
				if (unlikely(rbi->buffer == NULL)) {
				...
				rbi->dma_addr = dma_map_single(
						&adapter->pdev->dev,
						rbi->buffer + VMXNET3_FPB_OFFSET,
						rbi->len,
						PCI_DMA_FROMDEVICE);
				if (dma_mapping_error(&adapter->pdev->dev,
						      rbi->dma_addr)) {
					__skb_bin_put_buffer_noinvalidate(rbi->buffer);
					rq->stats.rx_buf_alloc_failure++;
					rbi->buffer = NULL;
					break;
				}
...
static struct vmxnet3_tx_queue* vmxnet3_fpb_tq(struct vmxnet3_adapter *adapter)
{
	return &adapter->tx_queue[fast_path_pick_tx(adapter->netdev)];
}

static int vmxnet3_parse_fpb(struct fp_buf *fpb, struct vmxnet3_tx_queue *tq,
			 struct vmxnet3_tx_ctx *ctx)
{
	int ret;
	...
	if (fpb_len(fpb) <= VMXNET3_HDR_COPY_SIZE) {
		ctx->copy_size = fpb_len(fpb);
		ret = 1;
	}
	else {
		ctx->copy_size = sizeof(struct ethhdr) + sizeof(struct iphdr);
		ctx->copy_size = min_t(unsigned int, ctx->copy_size,
				       fpb_len(fpb));
		ctx->copy_size = min_t(unsigned int, ctx->copy_size,
				       tq->txdata_desc_size);
		ret = VMXNET3_TXD_NEEDED(ctx->copy_size) +
			(ctx->copy_size < fpb_len(fpb));
	}
...
static int vmxnet3_map_fpb(struct fp_buf * fpb, struct vmxnet3_adapter *adapter,
	...
	if (ctx->copy_size) {
		tdd = (struct Vmxnet3_TxDataDesc *)((u8 *)tq->data_ring.base +
						    tq->tx_ring.next2fill *
						    tq->txdata_desc_size);
		memcpy(tdd->data, fpb_data(fpb), ctx->copy_size);
	...
	len = fpb_len(fpb) - ctx->copy_size;
	offset = ctx->copy_size;
	while (len) {
		if (len < VMXNET3_MAX_TX_BUF_SIZE) {
			chunk = len;
			dw2 |= len;
		} else {
			chunk = VMXNET3_MAX_TX_BUF_SIZE;
		}

		tbi = tq->buf_info + tq->tx_ring.next2fill;
		tbi->map_type = VMXNET3_MAP_SINGLE;
		tbi->dma_addr = dma_map_single(&adapter->pdev->dev,
					       fpb_data(fpb) + offset, chunk,
					       PCI_DMA_TODEVICE);
	...
	tbi->ptr = fpb_to_ptr(fpb);
	tbi->sop_idx = ctx->sop_txd - tq->tx_ring.base;

	return 0;
}

static void vmxnet3_flush_tx_deferred(struct vmxnet3_adapter *adapter,
				      struct vmxnet3_tx_queue *tq)
{
	tq->shared->txNumDeferred = 0;
	VMXNET3_WRITE_BAR0_REG(adapter,
...
static int vmxnet3_fast_path_xmit(struct net_device *dev, struct fp_buf *fpb)
{
	union Vmxnet3_GenericDesc *gdesc;
	struct vmxnet3_adapter *adapter;
	struct vmxnet3_tx_queue *tq;
	...
	tq = vmxnet3_fpb_tq(adapter);


	count = vmxnet3_parse_fpb(fpb, tq, &ctx);
	if (count <= 0) {
	...
	if (vmxnet3_map_fpb(fpb, adapter, tq, &ctx)) {
		goto unlock_and_drop;
	}
	...
	deferred = le32_to_cpu(tq->shared->txNumDeferred);
	...
	commit = !(tq->xmit_commits++);
	spin_unlock_irqrestore(&tq->tx_lock, flags);

	if (!deferred && commit) {
		schedule_xmit_commit(dev);
	} else if (deferred >= le32_to_cpu(tq->shared->txThreshold)) {
		vmxnet3_flush_tx_deferred(adapter, tq);
	}
...
static int vmxnet3_xmit_commit(struct net_device *dev) {
	struct vmxnet3_adapter *adapter;
	struct vmxnet3_tx_queue *tq;
...
inline bool
vmxnet3_allow_fast_path_rx(struct vmxnet3_adapter *adapter,
			   struct Vmxnet3_RxCompDesc *rcd) {
	return rcd->eop && !rcd->ts &&
		    (!rcd->tcp || !(adapter->netdev->features & NETIF_F_LRO));
}
		       ...
		       struct vmxnet3_adapter *adapter, int quota)
{
	...
	while (rcd->gen == rq->comp_ring.gen) {
		...
		if (rcd->sop) { /* first buf of the pkt */
			...
			BUG_ON(ctx->skb != NULL || rbi->buffer == NULL);
			...
			if (len + VMXNET3_FPB_OFFSET > skb_bin_data_size()) {
				netdev_warn(adapter->netdev,
					    "packet too big for fast-path (%d + %d > %u)\n",
					    len, VMXNET3_FPB_OFFSET, skb_bin_data_size());
				rq->stats.drop_total++;
				skip_page_frags = true;
				goto rcd_done;
			}

			new_buffer = __skb_bin_get_buffer();
			if (!new_buffer) {
				rq->stats.rx_buf_alloc_failure++;
				rq->stats.drop_total++;
				skip_page_frags = true;
			...
			if (rxDataRingUsed) {
				...
				fpb = fpb_build(new_buffer, VMXNET3_FPB_OFFSET,
						rcd->len);
				...
				fpb = fpb_build(rbi->buffer, VMXNET3_FPB_OFFSET,
						rcd->len);

				new_dma_addr = dma_map_single(
						&adapter->pdev->dev,
						new_buffer + VMXNET3_FPB_OFFSET,
						rbi->len,
						PCI_DMA_FROMDEVICE);
				if (dma_mapping_error(&adapter->pdev->dev,
						      new_dma_addr)) {
					__skb_bin_put_buffer_noinvalidate(new_buffer);
					rq->stats.rx_buf_alloc_failure++;
					rq->stats.drop_total++;
					skip_page_frags = true;
					goto rcd_done;
				...
				/* Immediate refill */
				rbi->buffer = new_buffer;
				rbi->dma_addr = new_dma_addr;
				rxd->addr = cpu_to_le64(rbi->dma_addr);
				rxd->len = rbi->len;
			}

			if (vmxnet3_allow_fast_path_rx(adapter, rcd)) {
	...
	for (ring_idx = 0; ring_idx < 2; ring_idx++) {
		for (i = 0; i < rq->rx_ring[ring_idx].size; i++) {
			...
			if (rxd->btype == VMXNET3_RXD_BTYPE_HEAD &&
					rq->buf_info[ring_idx][i].buffer) {
				dma_unmap_single(&adapter->pdev->dev, rxd->addr,
						 rxd->len, PCI_DMA_FROMDEVICE);
				__skb_bin_put_buffer_noinvalidate(rq->buf_info[ring_idx][i].buffer);
				rq->buf_info[ring_idx][i].buffer = NULL;
...
static int
vmxnet3_get_skb_bin_size(struct vmxnet3_adapter *adapter)
{
	int size;

	if (adapter->netdev->mtu <= VMXNET3_MAX_SKB_BUF_SIZE -
				    VMXNET3_MAX_ETH_HDR_SIZE) {
		size = adapter->netdev->mtu + VMXNET3_MAX_ETH_HDR_SIZE;
		if (size < VMXNET3_MIN_T0_BUF_SIZE)
			size = VMXNET3_MIN_T0_BUF_SIZE;
	} else {
		size = VMXNET3_MAX_SKB_BUF_SIZE;
	}

	return size + VMXNET3_FPB_OFFSET;
}
...
vmxnet3_open(struct net_device *netdev)
{
	...
	skb_bin_request(netdev, vmxnet3_get_skb_bin_size(adapter));
		     ...
		     const struct pci_device_id *id)
{
	static const struct net_device_ops vmxnet3_netdev_ops = {
		...
		.ndo_fast_path_xmit = vmxnet3_fast_path_xmit,
		.ndo_xmit_commit = vmxnet3_xmit_commit,
