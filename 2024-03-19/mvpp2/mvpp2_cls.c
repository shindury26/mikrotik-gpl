...
//#define DUMP_STATS
#ifdef DUMP_STATS
static struct timer_list statsTimer;
static struct mvpp2 *timer_priv;
...
static u32 mvpp2_cls_flow_hits(struct mvpp2 *priv, int index)
{
...
static const char *getFlowTypeName(unsigned e) {
	static char retb[32];
	switch(e) {
		case IPV4_FLOW: return "IPv4-none";
		case IPV6_FLOW: return "IPv6-none";
		case UDP_V4_FLOW: return "IPv4-UDP";
		case UDP_V6_FLOW: return "IPv6-UDP";
		case TCP_V4_FLOW: return "IPv4-TCP";
		case TCP_V6_FLOW: return "IPv6-TCP";
		case ETHER_FLOW: return "NO-IP";
	}
	sprintf(retb, "(??? %d)", e);
	return retb;
}

static void dump_flow_hits(struct mvpp2 *priv) {
	int i;

	printk("\n\n");
	for (i = 0 ; i < 512; i++) {
		unsigned hits = mvpp2_cls_flow_hits(priv, i);
		if(hits) {
			if (i >= ARRAY_SIZE(cls_flows)) {
				printk("flow-hits[%03d] count=%08d ?undefined\n",
					i, hits);
				continue;
			}
			struct mvpp2_cls_flow *flow = &cls_flows[i];
#define HAS_FLOW_BIT(bit) ( (flow->prs_ri.ri & (bit)) && (flow->prs_ri.ri_mask & (bit)) )

			printk("flow-hits[%03d] count=%08d %s %s\n",
				i, hits, getFlowTypeName(flow->flow_type),
				(HAS_FLOW_BIT(MVPP22_CLS_HEK_OPT_VLAN) ? "vlan" : "no-vlan")
			);
		}
	}
}

static void stats_work_handler(struct timer_list *t) {
	dump_flow_hits(timer_priv);
	mod_timer(&statsTimer, jiffies + 300);
...
static inline u32 mvpp22_rxfh_indir(struct mvpp2_port *port, u32 rxq)
{
	 ...
	 */
	return port->first_rxq + ((rxq * nrxqs + rxq / cpus) % port->priv->nthreads);
}
...
void mvpp22_rss_port_init(struct mvpp2_port *port)
{
	 ...
	 */
	for (i = 0; i < MVPP22_RSS_TABLE_ENTRIES; i++)
		port->indir[i] = ethtool_rxfh_indir_default(i, port->priv->nthreads);
...
#ifdef DUMP_STATS
	timer_priv = priv;
	timer_setup(&statsTimer, stats_work_handler, 0);
	mod_timer(&statsTimer, jiffies + 500);
#endif
}
