...
static bool ipt_psd_match(const struct sk_buff *pskb,
			  struct xt_action_param *par)
{
	...
	/* Sanity check */
	if (ntohs(iphdr->frag_off) & IP_OFFSET) {
	...
	tcp_hdr = (struct tcphdr*)((u_int32_t *)iphdr + iphdr->ihl);
	...
	*head = curr;
	...
	curr->timestamp = now;
	curr->src_addr = addr;
	curr->dest_addr.s_addr = iphdr->daddr;
