...
static int mcs7830_rx_fixup(struct usbnet *dev, struct fp_buf *buf)
{
	u8 status;
	...
	fpb_len_set(buf, fpb_len(buf) - 1);
	status = fpb_data(buf)[fpb_len(buf)];
	...
	return fpb_len(buf) > 0;
}

const struct driver_info moschip_info = {
...
const struct driver_info sitecom_info = {
...
struct usb_driver mcs7830_driver = {
	.name = driver_name,
	.id_table = products,
	.probe = usbnet_probe,
